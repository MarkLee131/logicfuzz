...
- **VALIDITY_CONTRACT** — valid-by-construction contract: every `nullable=False` opaque-handle arg gets a type-matching producer with type-correct binding (I1/I2a/I2b/I3) (`sequence_constructor._validity_contract`, `CBFactory._validity_contract_enabled`, `skeleton_generator._validity_contract_enabled`); lcms A/B: preflight-survived 1→14, merged distinct APIs 18→49.
- **POPULATE_COLLECTIONS** — render a CREATOR's handle-collection arg (`cmsToneCurve* const []`) as a populated array of built producer handles instead of degenerate `{0}`/NULL (`sequence_constructor._populate_collections`, `skeleton_generator`); lcms 300s-fuzz edges 325→943, cov 0.86%→16.81%.
...
- `LOGICFUZZ_SPEC_RENDER=1` — REPLACE #4: opt IN to the spec-guided LLM AUTHOR path. The Prototyper builds an author prompt from the serialized `render_spec` (a flat re-projection of the skeleton's `value_intents`+`api_sequence`+`includes`, attached at Step 10b by `hole_semantics.build_render_spec`/`annotate_skeletons`), AUTHORS a full driver via the existing `run_tool_calling_loop`, and gates it through `plan_conformance.analyze_conformance` (exact role hints from `render_spec["_role_hints"]`). Non-conformant ⇒ regenerate (≤2 retries, conformance diff fed back), then FALL BACK to the deterministic hole-fill path. Fail-open: flag OFF / no `render_spec` (SVF-degraded) / any exception ⇒ byte-identical deterministic path. Checked FIRST, independent of `LOGICFUZZ_LLM_REWRITE`. `src/agents/prototyper.py:_spec_render_enabled`/`_author_from_spec`. (DEFAULT-OFF, A/B pending — run A/B only on SVF-healthy libs cjson/lcms/libpng, the path is INERT where SVF fails.)
...
| **Merge pipeline (SSOT)** | `tools/merge_drivers/pipeline.py:run_merge_pipeline` | THE single merge orchestration both entry points call: `run_single_fuzz._maybe_merge_drivers` (production `--eval`/`--merge-drivers`) is a thin adapter, and the manual CLI `python -m tools.merge_drivers` delegates here too. Gate stack: collect-compiling → quarantine → preflight → orphan filter → compile-validate (A≡B) → **dominance-filter** → synthesize. Benchmark-agnostic (takes `project`/`stock_lang`/`iquote_dirs`/`out_dir` explicitly; the adapter owns `_stock_target_lang`/`_iquote_dirs_for_target`). |
| **Dominance-filter (merge selection)** | `tools/merge_drivers/select.py:dominance_filter` + `pipeline.py:_apply_dominance` | keep-all-non-dominated: drops a driver ONLY if its MEASURED reached-function set (`code-coverage-reports/<id>/linux/summary.json`) is a subset of another kept driver — provably preserves the distinct-API union (no flag; no-op keep-all fallback when <2 measured reports). Replaces the dead CLI-only greedy `select_top_k` (kept in reserve for a future hard cap). |
| **Pre-ship quarantine** | `tools/merge_drivers/pipeline.py:_is_immediate_crash_fp` + `_should_quarantine_from_merge` (called by the `_maybe_merge_drivers` adapter) | drops immediate-crash 0-coverage FP drivers from the merge; uses the trial's own verdict. + dead-filter bugfix (`dead_on_empty`). |
| **Compile-validation merge gate** | `tools/merge_drivers/compile_validate.py:validate_compilable` + `tools/merge_drivers/pipeline.py:_compile_validate_candidates` | merge includes ONLY drivers that COMPILE under real OSS-Fuzz cov-build flags (per-TU C/C++, `-Werror=implicit-function-declaration` re-promoted) so address-build and cov-build compile the IDENTICAL set (A≡B). Fail-open; opt-out `LOGICFUZZ_SKIP_COMPILE_VALIDATE=1`; writes `merged/compile_validation.json`. |
