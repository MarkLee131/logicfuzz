...
def _resolve_lang(src: Path, lang_override: Optional[str] = None) -> str:
    ...
    stock-target language (``lang_override``) when threaded in — so the gate, the
    merged build, and the per-trial build agree (A≡B), instead of content-sniffing
    an extensionless ``.fuzz_target`` and guessing wrong. ``None`` ⇒ per-driver
    sniff (standalone CLI with no benchmark)."""
    if lang_override:
...
def _include_search_dirs() -> Tuple[str, ...]:
    """In-image ``-I`` base dirs (``$PROJ`` is a shell var) probed for a
    project's PUBLIC headers. Too-LEAN drops drivers the real merged build would
    compile; over-inclusive ``-I`` is harmless for ``-fsyntax-only``. Covers
    common OSS-Fuzz layouts (lib/includes, inc, src/include); the script's
    dynamic find handles nesting this static list can't anticipate."""
    return (
...
set -u
PROJ="__PROJECT__"
# 1. GENERATE configure/cmake/autotools headers FIRST so the syntax check sees
# the SAME tree the real merged build compiles (A≡B). Fail-open: a failing/absent
# build must never drop drivers.
( compile >/dev/null 2>&1 || bash /src/build.sh >/dev/null 2>&1 || true )
# The heavyweight build can fail/time out before version/config headers exist; a
# LIGHTWEIGHT configure-only step produces them cheaply into a private build dir
# the include discovery below -I's. Fail-open.
if [ -f /src/$PROJ/CMakeLists.txt ] && command -v cmake >/dev/null 2>&1; then
  ( mkdir -p /src/$PROJ/_lf_genhdr && cd /src/$PROJ/_lf_genhdr \
    && cmake /src/$PROJ >/dev/null 2>&1 || true )
elif [ -x /src/$PROJ/configure ]; then
  ( cd /src/$PROJ && ./configure >/dev/null 2>&1 || true )
elif [ -f /src/$PROJ/configure.ac ] && command -v autoreconf >/dev/null 2>&1; then
  ( cd /src/$PROJ && autoreconf -i >/dev/null 2>&1 && ./configure >/dev/null 2>&1 || true )
fi
# 2. DISCOVER include dirs AFTER header generation (so generated build trees are
# picked up).
EXT_INC=""
...
# that nests its API headers resolves a plain ``#include <proj/x.h>`` the static
# list can't anticipate. Over-inclusive -I is harmless for -fsyntax-only.
for d in $(find /src/$PROJ -maxdepth 6 -type d \( -name include -o -name includes \) 2>/dev/null | sort -u); do
  EXT_INC="$EXT_INC -I$d"
done
...
def _build_validate_script(project: str,
    ...
    """Render the in-container validation bash for ``project`` — interpolating
    project name, ``-I`` search dirs, and ``-iquote`` bases. Pure (no I/O / no
    docker) so the include-discovery hardening is unit-testable."""
