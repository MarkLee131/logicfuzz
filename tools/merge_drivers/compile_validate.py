...
set -u
PROJ="__PROJECT__"
# === 1. GENERATE configure/cmake/autotools headers FIRST, before any include
# discovery, so the syntax check sees the SAME tree the real merged build compiles
# (A≡B). Best-effort + fail-open: a failing/absent build must never drop drivers.
( compile >/dev/null 2>&1 || bash /src/build.sh >/dev/null 2>&1 || true )
# The heavyweight build above can fail/time out at make/make check BEFORE the
# version/config headers are produced (nghttp2's build.sh runs `make check` =
# the whole test suite; nghttp2ver.h is generated by cmake's configure_file). A
# LIGHTWEIGHT configure-only step produces those headers cheaply (no compilation)
# into a private build dir the include discovery below then -I's. Fail-open.
...
# === 2. DISCOVER include dirs AFTER header generation (so generated build trees
# like /src/$PROJ/_lf_genhdr/lib/includes — holding nghttp2ver.h — are picked up).
EXT_INC=""
...
# that nests its API headers (nghttp2 lib/includes, the cmake build dir's
# generated lib/includes, *_src/include, …) resolves a plain ``#include
# <proj/x.h>`` the static list can't anticipate. Bounded depth + dedup keeps it
# cheap; over-inclusive -I is harmless for -fsyntax-only.
