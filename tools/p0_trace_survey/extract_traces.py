...
class FunctionBodyWalker:
    ...
    def __init__(self,
        ...
        self.var_to_call: Dict[str, str] = {}  # var_name → last call_id
    ...
    def _arg_var_name(self, arg_cursor: Cursor) -> Optional[str]:
        """If the arg is `varname` or `&varname` or `*varname`, return varname."""
    ...
    def _process_call(self, cursor: Cursor) -> Optional[CallSite]:
        ...
        for i, arg in enumerate(arg_children):
            ...
            elif var in self.var_to_call:
                # Read of a previously defined var → bind this arg slot.
                bindings[i] = self.var_to_call[var]
                if is_handle_slot:
                    n_bound_handle_here += 1
