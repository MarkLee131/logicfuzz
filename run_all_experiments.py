...
def main():
  ...
  # Library build-cache pre-pass (LOGICFUZZ_LIB_CACHE, default off). Compile each
  # project's library ONCE here — serially, before any trial pool — so the
  # per-trial builds only recompile the driver. No-op + fail-safe when disabled.
  if oss_fuzz_checkout.LIB_CACHE_ENABLED:
    oss_fuzz_checkout.prepare_library_cache(experiment_targets)
