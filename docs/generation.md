...
| **~~top_k breadth lever~~ (RETIRED 2026-06-20)** | none — `filter_top_k` defaults to `None` (no cap) | `data_context.py` (`prepare`), `coverage_ranker._greedy_select` | the `LOGICFUZZ_TOP_K` env knob was removed: it implied it capped API breadth, but breadth is portfolio-determined and construction + residual all-cover reach the extraction ceiling regardless of any cap (measured c-ares: 138 APIs / 73 drivers with the old `top_k=10`). The L4 greedy floor now self-terminates at full pool coverage; the fixed cap survives only as the `PORTFOLIO=off` A/B control | superseded by residual all-cover + portfolio API-floor (c-ares **138 ≥ 136**, cjson **78/78**) |
