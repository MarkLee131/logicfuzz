...
> **已否决（别再 re-litigate）**：**T9**（gap API 的静态 CFG 可达性权重）——离线 probe（已移除）实测 c-ares/lcms/zlib 依赖图很平（max depth 1-3），deep gap API 仅 7.1%；planner 盲点是 depth-无关的，重排权重救不了。根因是 **binding 层**（#14）。
