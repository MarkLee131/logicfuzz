"""Deterministic crash-triage routing (LEAN, now the default): _lean_crash_triage
maps the deterministic crash_frame verdict to the crash_analysis/context_analysis
the LLM agents would have written — library→keep(feasible), driver→fix(not
feasible), unknown→None(LLM fallback). no-crash / already-triaged → None.
