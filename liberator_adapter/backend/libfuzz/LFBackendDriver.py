...
class LFBackendDriver(BackendDriver):
    ...
    def __init__(self, working_dir, seeds_dir, num_seeds, headers_dir, public_headers):
        if public_headers is None:
            raise ValueError(
                "LFBackendDriver requires public_headers (path to "
                "public_headers.txt). The caller is responsible for ensuring "
                "the Clang/LLVM hybrid extractor produced this file BEFORE "
                "constructing the backend — see "
                "ProjectDriverGenerator.extract_metadata['local']['public_headers']. "
                "If extraction was skipped or failed, fail upstream rather "
                "than reaching the backend with None."
            )
        ...
        # Deterministic walk + sort so the generated driver's #include order is
        # stable across platforms / filesystems. ``os.walk`` itself returns
        # entries in directory-entry order, which differs across filesystems.
        self.headers = []
        for root, _, f_names in sorted(os.walk(headers_dir)):
    ...
    def emit_stub_functions(self, stub_functions: List[Function]) -> str:
        ...
        If a Function object has the ``stub_code`` attribute (enhanced
        stub generated by DriverEnhancer), use the enhanced stub;
        otherwise generate a default empty implementation.

        Upstream Liberator's body iterates via ``stub_functions.items()``
        — a dict-shaped iteration on a list (``Driver.stub_functions``
        is typed ``List[Function]``). The dead-code status of the
        backend masked the resulting AttributeError. Fixed here to
    ...
    def stmt_emit(self, stmt: Statement) -> str:
        ...
        raise NotImplementedError(
            f"stmt_emit: unhandled Statement subclass {type(stmt).__name__}"
        )
    ...
    def dyndblarrinit_emit(self, stmt: DynDblArrInit) -> str:
        ...
        if not buff_type.get_pointee_type().is_incomplete:
            ...
            raise Exception(
                f"dyndblarrinit_emit: pointee type {buff_type.get_pointee_type()} "
                f"is incomplete for buffer {buff_i}; cannot compute sizeof()"
            )
    ...
    def cleanbuffer_emit(self, cleanbuffer: CleanBuffer) -> str:
        ...
        # buffers in stack, heap, and global have different handling.
        # GLOBAL shares HEAP's array-of-pointers layout (see buffdecl_emit:
        # both branches emit ``T *token[1]...`` + a ``_shadow`` companion),
        # so the cleanup shape is identical. Upstream Liberator falls
        # through to ``return None`` for GLOBAL, which lets ``stmt_emit``
        # blow up with ``TypeError: can only concatenate str (not
        # "NoneType") to str`` — caught and fixed here.
        if buff.alloctype == AllocType.STACK:
            v_stack = self.clean_token(buff.token)
            return f"{cleanup_method}({v_stack});"
        elif buff.alloctype in (AllocType.HEAP, AllocType.GLOBAL):
            # add a shadow copy for the array
            x_idx = 0
            x_token = self.clean_token(buff.get_token()) + "_shadow"
            x_value = f"{x_token}[{x_idx}]"
            ...
            raise NotImplementedError(
                f"cleanbuffer_emit: unhandled AllocType {buff.alloctype!r}"
            )
    ...
    def dynarrayinit_emit(self, dynarrayinit: DynArrayInit) -> str:
        ...
        if (isinstance(buff_type, PointerType) and
            ...
            pointee = (buff_type.get_pointee_type()
                       if isinstance(buff_type, PointerType) else buff_type)
            raise Exception(
                f"dynarrayinit_emit: pointee type {pointee} is incomplete "
                f"for buffer {buff[0]}; cannot compute sizeof()"
            )
