...
def build_gap_reaching_seqs(
    ...
    *,
    ...
    """Uncovered-first (the union-breadth lever). For each gap API NOT already in
    the candidate pool, emit ONE best-effort carrier: a creator-prefix reaching
    chain if chainable (reusing the same deterministic prefix/closing helpers),
    else the API as a SINGLETON. These are KEPT verbatim (no ordering self-filter)
    — the unchecked render path turns the unbound/orphan arg into a hole for the
    LLM (pairs with B graceful degradation + the pre-ship quarantine, which drops
    any that end up immediate-crash FPs). Guarantees each gap API appears in at
    least one candidate, so the merged union grows toward PromeFuzz's ALL-COVER
    reach instead of saturating at the ~handful of chainable APIs. ``cap`` (>0)
    bounds how many are emitted (LLM/build cost control).

    Symbolic structure only (which producers, what lifecycle); the LLM still owns
    the leaf values it has to invent for the unbindable args.
    """
    gap_apis = gap_apis or set()
    already_covered = already_covered or set()
    idx = _build_index(model)
    missing = sorted({g for g in gap_apis if g in model.apis} - already_covered)
    out: List[List[str]] = []
    for name in missing:
        sem = model.apis[name]
        prefix, opened = _build_prefix(sem, idx, max_prefix_depth)
        opened = set(opened) | set(sem.produces)
        seq = prefix + [name] + _closing_destroyers(opened, idx)
        out.append(seq if seq else [name])
