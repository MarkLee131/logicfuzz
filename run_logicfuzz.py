...
def run_local_extraction_for_benchmark(
  ...
  # Build-first (option B), SCOPED to projects that declare ``public_headers`` in
  # their YAML: such a project's canonical public header may be GENERATED by the
  # build (sqlite3's amalgamation ``bld/sqlite3.h``) and is absent from the raw
  # checkout until the build runs — so the build must precede header use + clang
  # extraction. Projects relying on auto-discovery keep the ORIGINAL order
  # (discovery THEN build, below) → byte-identical, no regression. The bitcode
  # built here is REUSED by the LLVM-extraction block (no double build).
  llvm_extractor = None
  bc_file = None
  _force_build_first = os.environ.get(
      'LOGICFUZZ_FORCE_BUILD_FIRST', '').strip().lower() not in (
      '', '0', 'false', 'no', 'off')
  if enable_llvm_extraction and (
      getattr(benchmark, 'public_headers', None) or _force_build_first):
    llvm_extractor = LLVMAPIExtractor(benchmark, clang_extractor.container)
    logger.info('Build-first (YAML public_headers declared): compiling to '
                'bitcode so build-generated headers exist for clang extraction...')
    bc_file = llvm_extractor.compile_to_bitcode()
    logger.info(f'Generated bitcode file (build-first): {bc_file}')
  ...
  # Priority 0: USER-PROVIDED headers from the benchmark YAML (``public_headers``)
  # take precedence over auto-discovery and skip it entirely. Needed for projects
  # whose canonical public header is GENERATED by the build (sqlite3's amalgamation
  # ``sqlite3.h``) and so is absent from the raw checkout the priority search scans.
  # Generic: any project may list its public headers; projects that don't fall
  # through to discovery unchanged (empty list ⇒ no behavior change).
  _yaml_headers = list(getattr(benchmark, 'public_headers', None) or [])
  if _yaml_headers:
    headers_list = _yaml_headers
    logger.info(
        'Using %d public header(s) from benchmark YAML (skipping auto-discovery)'
        ': %s', len(headers_list), headers_list[:10])

  # Priority 1: Check include/ directory (most common location for public headers)
  include_dir = f'{project_dir}/include'
  if not headers_list and container.execute(f'test -d "{include_dir}" && echo "exists"').stdout.strip() == 'exists':
  ...
  if enable_llvm_extraction:
    ...
    if llvm_extractor is None:
      # Normal order (no YAML public_headers): build NOW, after clang extraction —
      # byte-identical to the pre-build-first behaviour.
      llvm_extractor = LLVMAPIExtractor(benchmark, clang_extractor.container)
      logger.info('Compiling project to bitcode with wllvm...')
      bc_file = llvm_extractor.compile_to_bitcode()
      logger.info(f'Generated bitcode file: {bc_file}')
    # else: bitcode was already built BEFORE clang extraction (build-first).
...
def run_experiments(benchmark: benchmarklib.Benchmark, args) -> Result:
  ...
  try:
    ...
    # Per-run LLM interaction ledger: record every call's prompt + response +
    # token usage to results/<proj>/llm_trace.jsonl (companion to the token
    # totals in token_summary.json). Configured before trials run so the
    # ThreadPool workers append into it.
    _proj = getattr(benchmark, 'project', None) or benchmark.id
    try:
      from src.utils import llm_trace as _lt
      _lt.configure(f"results/{_proj}/llm_trace.jsonl")
    except Exception:
      pass
    ...
    if _tm is not None:
      try:
        ...
        logger.info(
            ...
            _proj)
        logger.info('📝 [llm-trace] per-call prompt/response/token ledger → '
                    'results/%s/llm_trace.jsonl', _proj)
...
def parse_args() -> argparse.Namespace:
  ...
  # comprehender skip that API's LLM call) and @param/@return carry the
  # ownership / NULL contracts the generated driver needs. The former opt-in
  # flags (--use-doxygen-priors / --use-readme-purpose) were removed; behavior
  # lives in FuzzingContext.prepare() defaults.
...
def main():
    ...
    for benchmark in experiment_targets:
      ...
      if args.generate_drivers:
        if extraction_data.get('condition_manager') and extraction_data.get('function_conditions'):
          ...
          logger.warning('LLVM extraction must succeed to produce conditions.json (provenance data)')
    return
  ...
  # Idempotent + additive (no effect on fuzzer builds). See
